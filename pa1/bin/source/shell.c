...
int process_command(char **args)
{
  ...
  if (strcmp(args[0], "cd") == 0)
  {
    ...
    // Code for changing directory goes here
  }
  else if (strcmp(args[0], "help") == 0)
  {
    // Handle the help command
    // Code for displaying help information goes here
  }
  ...
  {
    // No built-in command matched, fork a child process
    pid_t pid = fork();
    ...
    if (pid == 0)
    {
      // Child process
      exec_sys_prog(args);
      // If exec_sys_prog returns, there was an error
      perror("exec_sys_prog failed");
      exit(1);
    }
    else if (pid > 0)
    {
...
char *read_line_stdin(void)
{
  ...
  /***** BEGIN ANSWER HERE *****/
  if (line == NULL) {
    perror("Error: Failed to allocate memory for line");
    exit(1);
  }

  ssize_t read_size = getline(&line, &buf_size, stdin);
...
void main_loop(void)
{
  ...
  do
  {
    ...
    /***** BEGIN ANSWER HERE *****/
    line = read_line_stdin();       // Step 1: Read line from stdin
    args = tokenize_line_stdin(line); // Step 2: Tokenize the line

    status = process_command(args); // Step 3: Execute the command

    free(line);  // Step 4: Free memory allocated for line

    free(args);  // Step 5: Free memory allocated for args
